        ...
        public async Task<IEnumerable<Medlem>> GetMedlemsList()
        {
            return await _context.Medlems
               .Include(x => x.Kontingent)
				...
				//.Include(x => x.KontingentForeignKey)
                .ToListAsync();
        }
...
public async Task<Medlem> GetMedlemByFornavn(string fornavn)
		...
		public async Task<Medlem> GetMedlemByFullnavn(string etternavn,  string fornavn)
        {
            return await _context.Medlems
                .Include(x => x.Kontingent)
				...
				.FirstOrDefaultAsync(x => x.Fornavn +  ' '  +  x.Etternavn  == fornavn  +  ' '  +  etternavn);
